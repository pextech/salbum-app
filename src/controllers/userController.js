...
function isValidName(fullnames) {
  if (fullnames.length >= 3 && fullnames != null) {
    return true;
  }
  return false;
}
...
export const createUser = async (req, res) => {
  if (!isValidName(req.body.fullnames)) {
  ...
  users.findOne({ where: { email: req.body.email } }).then((user) => {
    ...
    bcrypt.compare(req.body.password, user.password).then((result) => {
      if (result) {
        ...
        (res.status(200).json({ status: 200, message: `user ${user.fullnames} is logged in with below token`, token }));
      } else {
